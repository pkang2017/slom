...
def plot_islands_agreement(levels, image, label):
    ...
    check = ["airplane", "automobile", "bird", "cat", "deer", "dog", "frog", "horse", "ship", "truck"]

    fig, axs = plt.subplots(1, len(mylevels) + 1)
    plt.rcParams["figure.figsize"] = (30, 3)
    axs[-1].imshow(image_cpu)
    axs[-1].set_title('label: {}'.format(check[label.detach().cpu().numpy()]), fontsize=14)
    ...
    for i, matrice in enumerate(mylevels):
        ...
        axs[i].set_title('var: {}'.format(r.var()), fontsize=14)
        axs[i].set_box_aspect(1)
        axs[i].grid(False)
        axs[i].axes.xaxis.set_visible(False)
        axs[i].axes.yaxis.set_visible(False)
    path = "/home/pengkang/PycharmProjects/Agglomerator/islands/"+str(r.var())+".png"
    plt.savefig(path)
