...
class SpikingConvTokenizer(pl.LightningModule):
    ...
    def forward(self, x):
        out1 = self.block1(x)
        sp1 = self.sn1(out1)
        fr_sp1 = torch.count_nonzero(sp1)/torch.numel(sp1)
        out2 = self.block2(sp1)
        sp2 = self.sn2(out2)
        fr_sp2 = torch.count_nonzero(sp2) / torch.numel(sp2)
        out3 = self.block3(sp2)
        sp3 = self.sn3(out3)
        out = self.pool(sp3)
        return out, fr_sp1, fr_sp2
...
class SpikingColumnNet(pl.LightningModule):
    ...
    def forward(self, levels):
        if self.FLAGS.column_potential:
            temp = self.spiking_levels(levels)
            fr_temp = torch.count_nonzero(temp)/torch.numel(temp)
            levels_out1 = self.net1(temp)
            col_sp1 = self.col_sn1(levels_out1)
            fr_col_sp1 = torch.count_nonzero(col_sp1) / torch.numel(col_sp1)
            ...
            levels_out = self.rearrange(col_sp2)
        return levels_out, fr_temp, fr_col_sp1
...
class SpikingContrastiveHead(pl.LightningModule):
    def __init__(self, FLAGS, iters, num_patches, surrogate_fct=surrogate.ATan()):
        super().__init__()
        self.FLAGS = FLAGS
        self.iters = iters
        ...
        if self.FLAGS.spiking_neuron == "IFNeuron":
            self.cla_sn0 = neuron.IFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                     surrogate_function=surrogate_fct,
                                     detach_reset=self.FLAGS.detach_reset)
        elif self.FLAGS.spiking_neuron == "LIFNeuron":
            self.cla_sn0 = neuron.LIFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                      tau=self.FLAGS.tau, surrogate_function=surrogate_fct,
                                      detach_reset=self.FLAGS.detach_reset)
        elif self.FLAGS.spiking_neuron == "PLIFNeuron":
            self.cla_sn0 = neuron.PLIFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                       init_tau=self.FLAGS.tau, surrogate_function=surrogate_fct,
                                       detach_reset=self.FLAGS.detach_reset)
        else:
            print("No such neurons!")
        ...
        self.net1 = nn.Sequential(layer.Dropout(p=self.FLAGS.dropout), Rearrange('b n d -> b (n d)'),
        ...
        if self.FLAGS.spiking_neuron == "IFNeuron":
            self.cla_sn1 = neuron.IFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                     surrogate_function=surrogate_fct,
                                     detach_reset=self.FLAGS.detach_reset)
        elif self.FLAGS.spiking_neuron == "LIFNeuron":
            self.cla_sn1 = neuron.LIFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                      tau=self.FLAGS.tau, surrogate_function=surrogate_fct,
                                      detach_reset=self.FLAGS.detach_reset)
        elif self.FLAGS.spiking_neuron == "PLIFNeuron":
            self.cla_sn1 = neuron.PLIFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                       init_tau=self.FLAGS.tau, surrogate_function=surrogate_fct,
                                       detach_reset=self.FLAGS.detach_reset)
        else:
            print("No such neurons!")
        ...
        self.net2 = nn.Sequential(layer.Dropout(p=self.FLAGS.dropout), nn.Linear(self.num_patches * self.FLAGS.patch_dim, self.FLAGS.contr_dim),
                                  nn.LayerNorm(self.FLAGS.contr_dim))
        if self.FLAGS.contrast_potential:
            self.cla_sn2 = neuron.IFNode(v_threshold=float('inf'), v_reset=0.0, surrogate_function=surrogate_fct)
        else:
            if self.FLAGS.spiking_neuron == "IFNeuron":
                self.cla_sn2 = neuron.IFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                         surrogate_function=surrogate_fct,
                                         detach_reset=self.FLAGS.detach_reset)
            elif self.FLAGS.spiking_neuron == "LIFNeuron":
                self.cla_sn2 = neuron.LIFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                          tau=self.FLAGS.tau, surrogate_function=surrogate_fct,
                                          detach_reset=self.FLAGS.detach_reset)
            elif self.FLAGS.spiking_neuron == "PLIFNeuron":
                self.cla_sn2 = neuron.PLIFNode(v_threshold=self.FLAGS.v_th, v_reset=self.FLAGS.v_rst, \
                                           init_tau=self.FLAGS.tau, surrogate_function=surrogate_fct,
                                           detach_reset=self.FLAGS.detach_reset)
            else:
                print("No such neurons!")
    ...
    def forward(self, top_level):          # repeatly use last top level
        fr1, fr2 = 0, 0
        if self.FLAGS.contrast_potential:
            for _ in range(self.iters):
                temp_sp1 = self.cla_sn0(top_level)
                fr1 += torch.count_nonzero(temp_sp1) / torch.numel(temp_sp1)
                out1 = self.net1(temp_sp1)
                temp_sp2 = self.cla_sn1(out1)
                fr2 += torch.count_nonzero(temp_sp2) / torch.numel(temp_sp2)
                self.cla_sn2(self.net2(temp_sp2))
                sp_out = self.cla_sn2.v
        else:
            temp_sp1 = self.cla_sn0(top_level)
            fr1 += torch.count_nonzero(temp_sp1) / torch.numel(temp_sp1)
            out1 = self.net1(temp_sp1)
            temp_sp2 = self.cla_sn1(out1)
            fr2 += torch.count_nonzero(temp_sp2) / torch.numel(temp_sp2)
            sp_out = self.cla_sn2(self.net2(temp_sp2))
            for _ in range(1, self.iters):
                ...
                fr2 += torch.count_nonzero(temp_sp2) / torch.numel(temp_sp2)
                sp_out += self.cla_sn2(self.net2(temp_sp2))
            sp_out = sp_out / self.iters
        return sp_out, fr1/self.iters, fr2/self.iters


class SpikingClassificationHead(pl.LightningModule):
    def __init__(self, FLAGS, iters, surrogate_fct=surrogate.ATan()):
        super().__init__()
        self.FLAGS = FLAGS
        self.iters = iters
        ...
        self.net1 = nn.Sequential(layer.Dropout(p=self.FLAGS.dropout), nn.Linear(self.FLAGS.contr_dim, self.FLAGS.contr_dim),
                    nn.LayerNorm(self.FLAGS.contr_dim))
        ...
        self.net2 = nn.Sequential(layer.Dropout(p=self.FLAGS.dropout), nn.Linear(self.FLAGS.contr_dim, self.FLAGS.n_classes),
                    nn.LayerNorm(self.FLAGS.n_classes))
...
class SpikingGlom(pl.LightningModule):
    def __init__(self, FLAGS, *, consensus_self=False, local_consensus_radius=0, surrogate_function=surrogate.ATan()):
        ...
        self.token_rearrange = Rearrange('b d (h p1) (w p2) -> b (h w) (d p1 p2)', p1=self.FLAGS.patch_size, p2=self.FLAGS.patch_size)
        

        self.contrastive_head = SpikingContrastiveHead(self.FLAGS, self.FLAGS.contrast_time, self.num_patches, surrogate_fct=surrogate_function)
        self.classification_head_from_contr = SpikingClassificationHead(self.FLAGS, self.FLAGS.time, surrogate_fct=surrogate_function)
    ...
    def forward(self, img, levels=None):
        ...
        tokens, fr1, fr2 = self.image_to_tokens(img)
        tokens = self.token_rearrange(tokens)
        n = tokens.shape[1]
        ...
        fr_bu1, fr_bu2, fr_td1, fr_td2 = 0, 0, 0, 0
        for _ in range(self.iters):

            levels_with_input = torch.cat((bottom_level, levels.float()), dim=-2)

            bottom_up_out, temp_fr_bu1, temp_fr_bu2 = self.bottom_up(levels_with_input[..., :-1, :])
            fr_bu1 += temp_fr_bu1
            fr_bu2 += temp_fr_bu2

            top_down_out, temp_fr_td1, temp_fr_td2 = self.top_down(torch.flip(levels_with_input[..., 2:, :], [2]))
            fr_td1 += temp_fr_td1
            fr_td2 += temp_fr_td2
        ...
        top_level, fr1_con, fr2_con = self.contrastive_head(top_level)
        top_level = F.normalize(top_level, dim=1)          

        return top_level, fr1, fr2, fr_bu1, fr_bu2, fr_td1, fr_td2, fr1_con, fr2_con
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        if self.FLAGS.supervise:
            with torch.no_grad():
                top_level, fr1, fr2, fr_bu1, fr_bu2, fr_td1, fr_td2, fr1_con, fr2_con = self.forward(image)
                if self.FLAGS.mode == "energy":
                    self.fr1.append(fr1)
                    self.fr2.append(fr2)
                    self.fr_bu1.append(fr_bu1)
                    self.fr_bu2.append(fr_bu2)
                    self.fr_td1.append(fr_td1)
                    self.fr_td2.append(fr_td2)
                    self.fr1_con.append(fr1_con)
                    self.fr2_con.append(fr2_con)
        else:
            top_level, toplot = self.forward(image)
            if self.FLAGS.plot_islands:
                plot_islands_agreement(toplot, image[0, :, :, :], label[0])
        ...
        if self.FLAGS.supervise:
            output, fr1_cla, fr2_cla = self.classification_head_from_contr(top_level)
            self.fr1_cla.append(fr1_cla)
            self.fr2_cla.append(fr2_cla)
            loss = F.cross_entropy(output, label)
            self.batch_acc = self.accuracy(output.detach(), label, topk=(1,))[0]
