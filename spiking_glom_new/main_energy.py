...
def main(argv):
    ...
    if FLAGS.mode == "energy":
        ...
        for i, (d, m, f, l) in enumerate(zip(datasplits, modes, features_names, labels_names)):
            ...
            print("fr1_con", sum(model.fr1_con) / len(model.fr1_con))
            print("fr2_con", sum(model.fr2_con) / len(model.fr2_con))
            print("fr1_cla", sum(model.fr1_cla) / len(model.fr1_cla))
            print("fr2_cla", sum(model.fr2_cla) / len(model.fr2_cla))
